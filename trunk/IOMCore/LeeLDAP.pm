...
{ my $CONFIG=shift;
  ...
  { my (%aux,$attr,$claveind);
    ...
    %aux=();
    #Extrae el DN y lo aÒade siempre
    $aux{'dn'}{$entry->dn()}++;
...
Modificado para que emplee la versi√≥n provista por CVS

Revision 1.5  2008/04/22 10:27:28  calba
En SacaEntradaLDAP, el parametro campoindice, que marca la clave usada para el hash, pasa a ser opcional us√°ndose por defecto "dn".
...
- AÒadida funciÛn ValorLDAP:  a partir de una entrada saca el contenido de una variable. Si es una lista, hace un join con un separador que se pasa como parametro.
